...
const NUM_SIZE_CLASSES = {
  ...
  medium: "text-[clamp(1.25rem,5cqw,1rem)]",
};

export default function RatingBadge({ rating, size = "medium" }: Properties) {
  const color = ratingGradientColor(rating);
  ...
  return (
    ...
    >
      <div className="absolute inset-0 flex items-center justify-center">
        {/* Displayed value*/}
        <span
          className={`font-bold text-white [text-shadow:0_1px_1px_rgba(0,0,0,0.5)] ${NUM_SIZE_CLASSES[size]}`}
        >
          {displayValue}
        </span>
      </div>
